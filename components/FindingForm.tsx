...
const FindingForm: React.FC<FindingFormProps> = ({ onSubmit, onCancel, initialData, units }) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label htmlFor="unit-select" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
            <Building2 size={12} className="text-pepi-green-700" />
          ...
          <div className="relative group">
            <select 
              ...
              required
              name="unit"
              className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-pepi-green-500 focus:border-pepi-green-500 transition-all outline-none pr-10 cursor-pointer text-slate-700 font-medium"
              value={formData.unit}
            ...
            <ChevronDown size={18} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none group-focus-within:text-pepi-green-500 transition-colors" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label htmlFor="standar-select" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
            <BookOpen size={12} className="text-pepi-green-700" />
      ...
      <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-300">
        <div className="flex items-center justify-between">
          <label htmlFor="uraian-textarea" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
            <FileText size={12} className="text-pepi-green-700" />
        ...
        <div className="relative">
          <textarea 
            ...
            className="w-full bg-white border-2 border-slate-100 rounded-2xl px-5 py-4 text-sm focus:ring-4 focus:ring-pepi-green-500/10 focus:border-pepi-green-500 transition-all outline-none resize-none shadow-sm placeholder:text-slate-300 leading-relaxed font-medium"
      ...
      <div className="space-y-2">
        <label htmlFor="rencana-textarea" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <ClipboardCheck size={12} className="text-pepi-green-700" />
          Rencana Tindakan (Corrective Action)
          <span className="text-[9px] font-bold bg-pepi-warning-100 text-pepi-warning-800 px-2 py-0.5 rounded-full ml-auto">OPSIONAL</span>
        </label>
        <textarea 
          id="rencana-textarea"
          ...
          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-pepi-green-500 transition-all outline-none resize-none placeholder:italic"
      ...
      <div className="space-y-2">
        <label htmlFor="bukti-textarea" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
          <Paperclip size={12} className="text-pepi-green-700" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label htmlFor="tipe-select" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
            <AlertTriangle size={12} className="text-pepi-green-700" />
          ...
          <div className="relative group">
            <select 
              ...
              required
              name="tipe"
              className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-pepi-green-500 focus:border-pepi-green-500 transition-all outline-none pr-10 cursor-pointer text-slate-700 font-bold uppercase tracking-tight"
              value={formData.tipe}
              ...
              <label 
                ...
                className={`flex items-center justify-center p-2 rounded-xl border cursor-pointer transition-all text-[10px] font-black uppercase ${
                  formData.risiko === risk 
                    ? 'bg-pepi-green-900 border-pepi-green-900 text-white shadow-lg shadow-pepi-green-100' 
      ...
      <div className="pt-4 border-t border-slate-100">
        <div className="flex items-center justify-between gap-4 bg-slate-50 p-4 rounded-2xl">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white rounded-lg text-pepi-green-700 shadow-sm"><Clock size={18} /></div>
            <div>
          ...
          <select 
            id="status-select"
            name="status"
            className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold text-pepi-green-700 focus:ring-2 focus:ring-pepi-green-500 outline-none"
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button 
          type="submit"
          className="flex-[2] py-3 px-4 bg-pepi-green-900 hover:bg-pepi-green-800 text-white rounded-xl text-sm font-bold shadow-lg shadow-pepi-green-200 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
        >
          <Save size={18} />
